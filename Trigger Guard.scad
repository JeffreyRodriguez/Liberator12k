...
function GripPins() = [
   ...
   // Front-Bottom
   [GripTabBoltX(GripTabBoltsArray()[0]),GripFloorZ()-GripTriggerFingerSlotDiameter()+0.125],

   // Handle Bottom-Rear
   [GripHandleOffsetX()-2.528,GripFloorZ()-2.4],

   // Handle Bottom-Front
   [GripHandleOffsetX()-1.278,GripFloorZ()-2.9]
];
...
module GripTriggerFingerSlot(length=0.525, chamfer=false, $fn=Resolution(12, 60)) {
  render()
  union()
  translate([0.65,
             0,
...
module GripHandle(angle=25, length=1.3, height=5.5) {
  ...
  difference() {
    union() {
      ...
      // Backstrap
      translate([GripHandleOffsetX()-length-0.125,
                 0,
                 -GripFloor()-GripTriggerFingerSlotDiameter()])
      rotate([0,-5,0]) {
  ...
  difference() {
    union() {
      difference() {
        ...
        // Main block
        translate([GripTabRearMinX(), -GripWidth()/2, -height])
        cube([GripTabFrontMaxX()+abs(GripTabRearMinX())+0.5,
              GripWidth(),
              height]);
      ...
      hull() {
        ...
        // Front cube
        translate([GripTabFrontMaxX()+0.5,-0.625,-GripFloor()])
        mirror([1,0,0])
        cube([1.5, 1.25, GripFloor()]);

        // Rear cube
        translate([0.25,-0.5,-GripFloor()-GripTriggerFingerSlotRadius()])
        cube([GripTabFrontMaxX()+0.25, 1, GripFloor()+GripTriggerFingerSlotRadius()]);
      }

      // Rear Grip Tab Support
      hull() {

        translate([GripTabBoltX(GripTabBoltsArray()[1])-0.5,-GripWidth()/2,-0.49])
...
module GripAccessoryBossBolts(boltSpec=Spec_BoltM3(), length=UnitsMetric(24), clearance=true) {
  ...
  translate([GripTabFrontMaxX(),0, offsetZ])
...
module GripAccessoryBosses(holes=true, cutter=false) {
  ...
  for (offsetZ = GripAccessoryBossOffsets())
  difference() {
    translate([GripTabFrontMaxX()+0.5,-clearance, offsetZ-clearance])
    translate([0,-0.25,0])
...
module GripMiddle(showGripAccessory=true) {
  color("White")
  render()
  union() {
    difference() {
...
module Grip(showGripTabs=false, showGripTabBolts=false,
            showGripAccessory=true, showGripAccessoryBolts=false,
            showGripBodyBolts=false,
  ...
  if (showGripAccessory && showGripAccessoryBolts)
  ...
  if (showMiddle)
  GripMiddle(showGripAccessory=showGripAccessory);
...
Grip(showGripTabs=true, showGripTabBolts=true,
     showGripAccessory=false, showGripAccessoryBolts=true,
     showGripBodyBolts=true,
     showTrigger=true, showTriggerLeft=true, showTriggerRight=true,
        showLeft=false,
      showMiddle=true,
       showRight=true);
