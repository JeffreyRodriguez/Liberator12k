# [\#Liberator12k Manual](../README.md)
## Platform Components: Frame

![Frame](Frame.png)

The universal receiver in its "large frame" configuration.

## How it Works
The basic universal receiver, but now featuring two large frame bolts to support heavy forends.

## Bill of Materials

### Printed
179g, 15h06m

| Part                                                                                        | Settings       | Filament | Time    |
|---------------------------------------------------------------------------------------------|----------------|----------|---------|
| ![Frame_Receiver.stl](Frame_Receiver_thumb.png)<br>[Frame_Receiver.stl](Frame_Receiver.stl) | Standard       | 272g     | 1d3h59m |

### Hardware
| Purpose                | Part                             | Quantity |
|------------------------|----------------------------------|----------|
| Tension Bolt           | #8-32x12 Threaded Rod            | 4        |
| Tension Bolt Nut       | #8-32 Threaded Insert            | 4        |
| Tension Bolt Washer    | #8-32 Washer                     | 4        |
| Tension Bolt Acorn Nut | #8-32 Acorn Nut                  | 4        |
| Lower Retention Pin    | Universal Clevis Pin 1/4x2"      | 1        |
| Frame Bolt             | 1/2-13x10 Hex Bolt               | 2        |
| Frame Nut              | 1/2-13 Hex Nut                   | 4        |

### Components
 * ![Lower](Lower/Lower_thumb.png)<br>[Lower](Lower/Lower.md)
 * ![Trigger](Lower/Trigger_thumb.png)<br>[Trigger](Lower/Trigger.md)
 * ![Receiver](Lower/LowerMount_thumb.png)<br>[LowerMount](Lower/LowerMount.md)
 
## Assembly

#### Tension Nuts
Insert a threaded rod through the receiver from the rear and screw on the threaded insert while you install the threaded inserts, this will help keep them aligned.