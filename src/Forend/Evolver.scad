...
function Evolver_ActuatorScrewZ() = BarrelRadius()+0.375;
...
function Evolver_ChargerPinionTeeth() = 11;
function Evolver_ChargerPinion2Teeth() = 25;
function Evolver_ChargerGearPitch() = 0.125;
function Evolver_ChargerGearRatio() = Evolver_ChargerPinion2Teeth() / Evolver_ChargerPinionTeeth();
echo("Evolver_ChargerGearRatio", Evolver_ChargerGearRatio());
...
function Evolver_ChargerPinion2OutsideRadius(clearance=0) = outer_radius(Evolver_ChargerGearPitch(), Evolver_ChargerPinion2Teeth(), clearance);
...
function Evolver_ChargerPinionX() = 4;//2.5;
function Evolver_ChargerPinionY() = -0.3125;
...
module Evolver_BarrelCollar(clearance=BARREL_CLEARANCE, cutter=false, alpha=1, cutaway=false) {
  ...
  rotate([0,90,0])
  difference() {
    cylinder(r=BarrelCollarRadius()+clear, h=BarrelCollarLength()+clear2);
...
module Evolver_ChargerPinionPin(cutter=false, clearance=0.004) {
  clear = cutter? clearance : 0;
  clear2 = clear*2;

  color("Silver") RenderIf(!cutter)
  translate([Evolver_ChargerPinionX(),Evolver_ChargerPinionY(),Evolver_ChargerZ()-0.125-clear])
  cylinder(r=(Millimeters(2.5)/2)+clear, h=1/2+clear2);
}
...
module Evolver_BarrelSupport(length=Evolver_BarrelSupportLength(), doRender=true, cutaway=false, alpha=_ALPHA_FOREND_SPACER) {
  ...
  difference() {
    union() {
      translate([Evolver_BarrelSupportMinX(),0,0])
      Frame_Receiver_Segment(length=length,
      ...
      hull() {
        ...
        // Spindle support
        translate([Evolver_BarrelSupportMinX(),0,SpindleZ()])
        rotate([0,90,0])
        ChamferedCylinder(r1=0.5+0.125, r2=CR, h=length,
                          teardropTop=true);
        ...
        *translate([Evolver_BarrelSupportMinX(),0,SpindleZ()])
        ...
        // Pawl Pivot Support
        translate([Evolver_BarrelSupportMinX(),0,pawlPivotZ])
        rotate([0,90,0])
        ...
        translate([Evolver_BarrelSupportMinX(),-(1/2), pawlPivotZ+0.1875])
        ChamferedCube([1, 1, 0.5], r=1/16);
      }

      // Screw support
      for (R = [Evolver_ActuatorScrewAngle(),-Evolver_ActuatorScrewAngle()]) rotate([R,0,0])
      translate([Evolver_BarrelSupportMinX(),0, Evolver_ActuatorScrewZ()])
      rotate([0,90,0])
      ChamferedCylinder(r1=0.5,r2=1/16, h=length);
    }
    ...
    translate([Evolver_BarrelSupportMinX()+length,0,0])
    rotate([0,-90,0])
    HoleChamfer(r1=BarrelCollarRadius(), r2=internalCR, teardrop=true);

    // Actuator screw chamfers
    for (R = [Evolver_ActuatorScrewAngle(),-Evolver_ActuatorScrewAngle()]) rotate([R,0,0])
    translate([Evolver_BarrelSupportMinX()+length,0, Evolver_ActuatorScrewZ()])
    rotate([0,-90,0])
...
module Evolver_ActuatorToggle(AF=0, stopTab=true, cutter=false, clearance=0.01, cutaway=false, alpha=1) {
  ...
  stopLength = 0.5+0.125;
  width = 0.25;
  height = 0.125;
  wall = NozzleMultiple(6);
  toggleAngle=-15;
  helixOverAngle = SegmentAngle((width/4), BarrelCollarRadius()+0.1875);
  tabOffsetX = Evolver_ActuatorMinX()+0.5-clear;// Evolver_BarrelSupportMinX
  length = ActuatorTabLength();
  camBodyAngle = 60;
  camBodyLength = 2;
  bodyLength = camBodyLength;
  innerRadius = BarrelRadius();
  extensionRadius = BarrelRadius()+wall+clear;
  extensionAngle = 180-abs(cutter?toggleAngle*6:toggleAngle*4);
  helixOffsetX = tabOffsetX+stopLength;
    ...
    union() {
      ...
      difference() {
        union() {
          ...
          intersection()  {
            ...
            rotate([-60-toggleAngle+(cutter?2:0),0,0])
            translate([0,-(width/2)+(cutter?clearance*2:clearance), 0])
            ChamferedCube([stopLength+(cutter?tabOffsetX:0)+clear2,
            ...
            translate([(cutter?0:tabOffsetX)-clear,0,0])
            rotate([0,90,0])
            cylinder(r=BarrelCollarRadius()+0.1875+clear, h=stopLength+(cutter?tabOffsetX:0)+clear2);
          }
        }

        // Cam helix cut
        if (!cutter)
        for (X = [0:width/2:ActuatorTabLength()-(width*1.4)])
        ...
        difference() {
          rotate([-60,0,0])
          translate([0,-(width/2)-clearance, 0])
      ...
      intersection() {
        ...
        semicircle(od=(extensionRadius+clear)*2,
                  angle=extensionAngle+(cutter?abs(toggleAngle*3):0), center=false);
      }
...
module Evolver_PumpRods(doMirror=true, cutter=false, innerCut=false, clearance=0.01, cutaway=false, alpha=1) {
  ...
  difference() {
    union() {
      ...
      // Long rod
      translate([pumpPinX-ActuatorTabLength()-clear,-(width/2)-clear, (innerCut?0:BarrelCollarRadius())-clear])
      ChamferedCube([length+ActuatorTabLength()+CR*2+clear2, width+clear2, (innerCut?+BarrelCollarRadius():0)+height+clear2],
      ...
      ChamferedCube([extend+(cutter?Evolver_BarrelTravel():0)+clear2,
                    ...
                    r=1/16, teardropFlip=[true,false,false],
                    disabled=cutter);
...
module Evolver_BarrelRack(clearance=0.008, cutter=false) {
  ...
  thickness = 0.25;
  teeth=ceil(Evolver_ChargerTravel(2)/Evolver_ChargerGearPitch())+1;
  rackMinX = Evolver_ChargerPinionX()-Evolver_ChargerTravel(2);
  modVal = module_value(Evolver_ChargerGearPitch());
  cutTravel = cutter ? ForendLength() : 0;
  offsetY = Evolver_ChargerPinionY()+Evolver_ChargerPinion2PitchRadius();
  offsetZ = Evolver_ChargerZ()+thickness;
  ...
  union() {
    ...
    // Rack
    if (!cutter)
    translate([rackMinX,offsetY,offsetZ-(thickness/2)])
    translate([0,0,height]) rotate([180,0,0])
    rack(mm_per_tooth = Evolver_ChargerGearPitch(), number_of_teeth=teeth, thickness=thickness, height=height);

    // Collar connection bar
    difference() {
      union() {

        // Rack extension
        translate([rackMinX-Evolver_ChargerGearPitch(),thickness+offsetY+clear,
                   offsetZ-clear])
        mirror([0,1,0])
        ChamferedCube([Evolver_ChargerTravel(2)+(offsetY*2)+cutTravel,
                       thickness+(cutter?modVal*2:-modVal)+clear2,
                      height+clear2], r=cr, disabled=cutter);

        // Forward extension
        translate([Evolver_ChargerPinionX(),thickness+offsetY+clear,offsetZ-clear])
        mirror([0,1,0])
        ...
        // Handlebar
        translate([ForendLength()+BarrelCollarOffset()+1,thickness+offsetY+clear-0.5,offsetZ-clear])
        mirror([1,0,0])
        ChamferedCube([thickness+clear2,
                       1+clear2,
                       height+clear2], r=cr, disabled=cutter);
...
module Evolver_ChargerRack(clearance=0.008, cutter=false) {
  ...
  cr = 1/32;
  modVal = module_value(Evolver_ChargerGearPitch());
  rackOffsetY = Evolver_ChargerPinionY()-Evolver_ChargerPinionPitchRadius();
  ...
  color("Olive") RenderIf(!cutter) {
    ...
    // Rack
    if (!cutter)
    translate([Evolver_ChargerPinionX(),rackOffsetY,Evolver_ChargerZ()+(thickness*0.5)])
    rack(mm_per_tooth = Evolver_ChargerGearPitch(),
    ...
    difference() {
      union() {
        ...
        // Rack support
        translate([Evolver_ChargerPinionX()-cutTravel,rackOffsetY-thickness-clear,Evolver_ChargerZ()-clear])
        ChamferedCube([Evolver_ChargerTravel()+cutTravel+overcut,
                      thickness+(cutter?modVal:-modVal)+clear2,
                      height+clear2], r=cr, disabled=cutter);
        ...
        hull() {
          ...
          // Rack section
          translate([Evolver_ChargerPinionX()-0.625-cutTravel,rackOffsetY-thickness-clear,Evolver_ChargerZ()-clear])
      ...
      // Pinion hole
      if (!cutter)
      translate([Evolver_ChargerPinionX()-(Evolver_ChargerGearPitch()*2),Evolver_ChargerPinionY(),Evolver_ChargerZ()])
      ChamferedCircularHole(r1=Evolver_ChargerPinionOutsideRadius(), r2=CR(), h=thickness);

      if (!cutter)
      translate([Evolver_ChargerPinionX()-(Evolver_ChargerGearPitch()*2),rackOffsetY-modVal-clearance,Evolver_ChargerZ()])
      cube([2, Evolver_ChargerPinionOutsideRadius()*2, thickness]);
    }
  }
}
...
module Evolver_ChargerPinion(af=0, clearance=0.008, cutter=false) {
  ...
  if (cutter) {
    translate([Evolver_ChargerPinionX(),Evolver_ChargerPinionY(),Evolver_ChargerZ()-clear])
    cylinder(r=Evolver_ChargerPinionOutsideRadius()+clearance, h=thickness+clear2);
  } else union() {
    // Big gear
    intersection() {
      translate([Evolver_ChargerPinionX(),Evolver_ChargerPinionY(),(thickness/2)+thickness+Evolver_ChargerZ()-clear])
      rotate(360/Evolver_ChargerPinion2Teeth()/2)
      rotate(-(360*Evolver_ChargerTravel()/Evolver_ChargerPinionPitchCircumference())*af)
      gear (mm_per_tooth    = Evolver_ChargerGearPitch(),
            number_of_teeth = Evolver_ChargerPinion2Teeth(),
            thickness       = thickness,
            hole_diameter   = hole_diameter,
            clearance = clearance);

      translate([Evolver_ChargerPinionX(),Evolver_ChargerPinionY(),Evolver_ChargerZ()+thickness-clear])
      ChamferedCylinder(r1=Evolver_ChargerPinion2OutsideRadius()+clearance, r2=cr, h=thickness+clear2);
    }

    // Small gear
    intersection() {
      translate([Evolver_ChargerPinionX(),Evolver_ChargerPinionY(),(thickness/2)+Evolver_ChargerZ()])
      rotate(360/Evolver_ChargerPinionTeeth()/2)
      rotate(-(360*Evolver_ChargerTravel()/Evolver_ChargerPinionPitchCircumference())*af)
      gear (mm_per_tooth    = Evolver_ChargerGearPitch(),
            number_of_teeth = Evolver_ChargerPinionTeeth(),
            thickness       = thickness,
            hole_diameter   = hole_diameter,
            clearance = clearance);
...
module Evolver_ChargerPreloader(af=0, clearance=0.008, cutter=false) {
  ...
  cr = 1/16;
  offsetY = Evolver_ChargerPinionY()-Evolver_ChargerPinionPitchRadius();

  color("CornflowerBlue") render()
  union() {

    // Pusher
    translate([Evolver_ChargerPinionX()+Evolver_ChargerTravel(),offsetY+clear,Evolver_ChargerZ()-clear])
    mirror([0,1,0])
    ChamferedCube([1.25+(cutter?0.25:0)+cutTravel, thickness+clear2, thickness+clear2], r=cr, disabled=cutter);

    // Captured tab
    translate([Evolver_ChargerPinionX()+Evolver_ChargerTravel()+0.3125-Evolver_ChargerGearPitch()-clear,clear,Evolver_ChargerZ()-clear])
    mirror([0,1,0])
...
module EvolverForendAssembly(hardware=true, prints=true, pipeAlpha=1, cutaway=false) {
  ...
  animateCharger = SubAnimate(ANIMATION_STEP_UNLOAD, end=0.5)
                - SubAnimate(ANIMATION_STEP_LOAD, start=0.5);

  animateActuatorToggle = SubAnimate(ANIMATION_STEP_EXTRACT, end=0.05)
                        - SubAnimate(ANIMATION_STEP_LOAD, start=0.15, end=0.2);

  if (_SHOW_CHARGER && _SHOW_HARDWARE)
  Evolver_ChargerPinionPin();

  if (_SHOW_SPINDLE && _SHOW_HARDWARE)
  Evolver_SpindleRod();
  ...
  if (prints && _SHOW_CHARGER_PINION)
  ...
  translate([Evolver_ActuatorTravel()*animateBarrel2,0,0]) {
    ...
    Evolver_ActuatorToggle(AF=animateActuatorToggle);
...
if ($preview) {
  translate([-ReceiverFrontLength(),0,0]) {
    ...
    if (_SHOW_STOCK)
    StockAssembly(hardware=_SHOW_HARDWARE, prints=_SHOW_PRINTS, alpha=_ALPHA_STOCK);
  }
  ...
  if (_RENDER == "Prints/ChargerPinion")
    if (!_RENDER_PRINT)
