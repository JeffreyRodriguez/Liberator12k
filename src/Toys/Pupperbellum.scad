...
BRANDING_MODEL_NAME = "Pupperbellum";
...
module ShaftCollar(cutter=false, clearance=0.02) {
  ...
  rotate([0,90,0])
    ...
    ABSMG9_Barrel(cutter=true, hollow=false);
  }
}

module ABSMG9_Magazine() {
...
module ABSMG9_Barrel(cutter=false, hollow=true) {
...
module ABSMG9_Forend(cutaway=false, alpha=1) {
...
module ABSMG9(hardware=_SHOW_HARDWARE, prints=_SHOW_PRINTS,
                      cutaway=false) {
...
if ($preview) {
  ...
  ABSMG9(hardware=_SHOW_HARDWARE, prints=_SHOW_PRINTS,
                        cutaway=false);
  if (_SHOW_FCG)
...
//
// Lower attachment lugs and mounting plate
//
*!scale(25.4)
